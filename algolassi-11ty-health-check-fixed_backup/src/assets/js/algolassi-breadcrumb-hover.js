...
(function () {
  ...
  var reopenChildMenuAfterNavigation = false;
  ...
  function reopenCurrentChildMenu() {
    if (!reopenChildMenuAfterNavigation) return;

    var selected = document.querySelector(".breadcrumb-child-menu a.breadcrumb-dropdown-current");
    if (!selected) {
      reopenChildMenuAfterNavigation = false;
      return;
    }

    var menu = selected.closest(".breadcrumb-child-menu");
    if (!menu) {
      reopenChildMenuAfterNavigation = false;
      return;
    }

    var item = menu.closest(".breadcrumb-child-item");
    if (item) {
      closeAllMenus(menu);
      constrainChildMenuWidth(item);
      alignChildMenu(item);
  ...
  document.addEventListener("click", function (event) {
    var link = event.target && event.target.closest ? event.target.closest(".breadcrumb-child-menu > a") : null;
    if (link) {
      reopenChildMenuAfterNavigation = true;
    }
  }, true);
  ...
  document.addEventListener("click", function (event) {
    ...
    var trigger = event.target && event.target.closest
      ? event.target.closest(".breadcrumb-child-item > .breadcrumb-trigger, .breadcrumb-child-item.breadcrumb-current, .breadcrumb-item > .breadcrumb-trigger")
      : null;

    var openMenu = document.querySelector(".breadcrumb-child-menu.open");

    if (trigger) {
      var item = trigger.closest(".breadcrumb-child-item") || trigger.closest(".breadcrumb-item");
      if (!item) return;
