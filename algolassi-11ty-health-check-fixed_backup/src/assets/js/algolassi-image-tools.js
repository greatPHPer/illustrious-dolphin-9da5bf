...
(function () {
  ...
  function setStatus(text, good) {
    var el = q("image-status"); if (!el) return;
    el.textContent = text || ""; el.classList.toggle("image-status-good", !!good);
  }
  ...
  function canvasBlob(canvas, type, quality) {
    return new Promise(function (resolve, reject) {
      canvas.toBlob(function (blob) { blob ? resolve(blob) : reject(new Error("Unsupported output format")); }, type, quality);
    });
  }
  function canvasFromItem(item) {
    return loadImageFromUrl(item.objectUrl).then(function (img) {
      ...
      c.getContext("2d", {willReadFrequently:true}).drawImage(img, 0, 0, item.width, item.height); return c;
    });
  }
  ...
  function formatOptions(item) {
    return ["image/png","image/jpeg","image/webp","image/avif"].map(function (type) {
      return {type:type, label:fileExt(type).toUpperCase(), selected:(type === item.type || (item.type === "image/gif" && type === "image/png"))};
    });
  }
  function exportItem(item, type) {
    if (type === item.type) { downloadBlob(item.blob, normalizeBase(item.name) + "." + fileExt(type)); return; }
    canvasFromItem(item).then(function (canvas) { return canvasBlob(canvas, type, .92); })
      .then(function (blob) { downloadBlob(blob, normalizeBase(item.name) + "." + fileExt(type)); })
      .catch(function () { setStatus("That output format is not supported by this browser.", false); });
  }
  function renderHistory() {
    var history=q("image-history"), empty=q("image-history-empty"); if(!history)return;
    history.innerHTML="";
    if(!state.items.length){if(empty)empty.classList.remove("image-hidden");return;}
    if(empty)empty.classList.add("image-hidden");

    state.items.forEach(function(item,index){
      if(index){var connector=document.createElement("div");connector.className="image-history-connector";connector.setAttribute("aria-hidden","true");history.appendChild(connector);}
      var card=document.createElement("article"); card.className="image-history-card"+(index===state.current?" current":""); card.dataset.index=index;
      var button=document.createElement("button"); button.type="button"; button.className="image-history-link"; button.setAttribute("aria-label","View stage "+(index+1)+": "+item.operation);
      ...
      var format=document.createElement("select"); format.setAttribute("aria-label","Download format for stage "+(index+1));
      formatOptions(item).forEach(function(opt){var o=document.createElement("option");o.value=opt.type;o.textContent=opt.label;o.selected=opt.selected;format.appendChild(o);});
      ...
      var use=document.createElement("button");use.type="button";use.textContent="Use for editing";use.addEventListener("click",function(){state.current=index;renderHistory();showCurrent();menuWrap.classList.remove("open");});
  ...
  function showCurrent(){
    var item=currentItem(),img=q("image-preview-img"),empty=q("image-preview-empty"),dims=q("image-dimensions"); if(!img)return;
    if(!item){img.removeAttribute("src");img.classList.add("image-hidden");if(empty)empty.classList.remove("image-hidden");if(dims)dims.textContent="No image selected";return;}
    img.src=item.objectUrl;img.classList.remove("image-hidden");if(empty)empty.classList.add("image-hidden");if(dims)dims.textContent=item.width+" × "+item.height+" • "+fileExt(item.type).toUpperCase();
  }
  ...
  function scaleCurrent(){
    ...
    processCurrent("Scaled to "+width+"×"+height,function(it){return canvasFromItem(it).then(function(c){var out=document.createElement("canvas");out.width=width;out.height=height;out.getContext("2d").drawImage(c,0,0,width,height);return canvasBlob(out,"image/png").then(function(blob){return {blob:blob,name:normalizeBase(it.name)+"-scaled.png",width:width,height:height};});});});
  }
  ...
  function clearHistory(){state.items.forEach(revokeItem);state.items=[];state.current=-1;renderHistory();showCurrent();setStatus("");}
  ...
  function init(){
    ...
    q("image-scale-button").addEventListener("click",scaleCurrent);q("image-crop-button").addEventListener("click",cropCurrent);q("image-transparent-button").addEventListener("click",transparentCurrent);q("image-clear-button").addEventListener("click",clearHistory);
    ...
    document.addEventListener("click",function(e){if(!e.target.closest(".image-card-menu"))document.querySelectorAll(".image-card-menu.open").forEach(function(x){x.classList.remove("open");});});
    renderHistory();showCurrent();
  }
