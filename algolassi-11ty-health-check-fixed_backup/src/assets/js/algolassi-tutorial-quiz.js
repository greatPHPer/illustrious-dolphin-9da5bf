...
(function () {
  ...
  function makeQuestionFromLine(line) {
    ...
    if (match) {
      ...
      if (/^(WriteLine|ReadLine)$/.test(method)) {
        ...
        if (prefix) {
          ...
          return {
            display: original.slice(0, idx) + "_____" + original.slice(idx + prefix.length),
            answer: prefix,
            source: original
          };
        }
    ...
    if (call && call[1].length >= 4) {
      ...
      if (prefix.length >= 2) {
        var start = original.indexOf("." + token) + 1;
        return {
          display: original.slice(0, start) + "_____" + original.slice(start + prefix.length),
          answer: prefix,
          source: original
        };
      }
    ...
    if (words) {
      for (var i = 0; i < words.length; i++) {
        var word = words[i];
        if (/^(using|namespace|public|private|protected|static|class|void|string|int|var|return|new|true|false|null|async|await|Task)$/.test(word)) continue;
        if (!/[A-Z_]/.test(word)) continue;
        var pos = original.indexOf(word);
        return {
          display: original.slice(0, pos) + "_____" + original.slice(pos + word.length),
          answer: word,
          source: original
        };
      }
  ...
  async function getSharedClient() {
    ...
    if (!window.AlgolassiSupabase) {
      window.AlgolassiSupabase = module.createClient(url, key, {
  ...
  function renderQuiz(question) {
    ...
    async function doCheck() {
      ...
      if (answer === currentQuestion.answer) {
        ...
        try {
          var reward = await awardReputation(currentQuestion.id);
          var ps = pageState(currentKey);
          ...
          result.textContent = reward.awarded === false
            ? "Already rewarded — reputation is safe."
            : "Correct! +" + REWARD + " reputation";

          window.dispatchEvent(new CustomEvent("algolassi:tutorial-quiz-correct", {
            detail: { quizId: currentQuestion.id, points: reward.awarded === false ? 0 : REWARD }
          }));
        } catch (e) {
          ...
          } else if (e && e.message && /function .*award_tutorial_quiz_reputation.*does not exist|Could not find the function/i.test(e.message)) {
            result.textContent = "Correct! Reputation reward is not enabled yet.";
          } else {
            result.textContent = "Correct! Reputation could not be added.";
          }
  ...
  function maybeQuiz() {
    ...
    if (ps.lastCoverageAtPrompt &&
        coverage - ps.lastCoverageAtPrompt < (document.documentElement.scrollHeight || 1) * MIN_ADDITIONAL_COVERAGE) return;
