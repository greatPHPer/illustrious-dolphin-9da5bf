...
(function () {
  ...
  function historyOperation(button) {
    ...
    var id = button.id || "";
    if (/^image-(scale-preview|scale|crop|transparent|rotate-90|rotate-180|rotate-270|save-as)-button$/.test(id)) return (button.textContent || id).trim();
    return "";
  }
  ...
  function bindImageHistory() {
    ...
    document.addEventListener("click", function (event) {
      ...
      window.setTimeout(function () {
        if (window.AlgolassiToolHistory && typeof window.AlgolassiToolHistory.add === "function") window.AlgolassiToolHistory.add(operation);
      }, 180);
    });
  }
  ...
  function bindImageMenuHoverBehaviour() {
    ...
    document.addEventListener("pointerover", function (event) {
      var trigger = event.target && event.target.closest ? event.target.closest(".image-menu-trigger") : null;
      if (!trigger || !trigger.closest(".image-workspace")) return;
      if (!window.matchMedia("(hover: hover) and (pointer: fine)").matches) return;
      var related = event.relatedTarget;
      if (related && trigger.contains(related)) return;
      if (trigger.id === "image-file-menu-button") openImageMenu("image-file-menu", "image-file-menu-button");
      else if (trigger.id === "image-edit-menu-button") openImageMenu("image-edit-menu", "image-edit-menu-button");
    }, true);
    document.addEventListener("click", function (event) {
      var target = event.target;
      if (!target || !target.closest || !target.closest(".image-menu-bar")) return;
      var keepOpen = target.closest(".image-menu-trigger, .image-menu-dropdown, .image-menu-item, .image-menu-clear");
      if (!keepOpen) closeImageMenus();
    }, true);
  }

  function syncCropOverlayAlignment() {
    var stage = q("image-preview-stage"), img = q("image-preview-img"), box = q("image-crop-rectangle");
    if (!stage || !box) return;
    if (!img || img.classList.contains("image-hidden") || !img.naturalWidth) {
      if (stage.classList.contains("image-layers-active")) {
        box.style.transform = "none"; box.style.margin = "0"; box.style.padding = "0"; box.style.boxSizing = "border-box";
      }
      return;
    }
    var stageRect = stage.getBoundingClientRect(), imageRect = img.getBoundingClientRect();
    box.style.transform = "translate(" + Math.round(imageRect.left - stageRect.left) + "px," + Math.round(imageRect.top - stageRect.top) + "px)";
    box.style.margin = "0"; box.style.padding = "0"; box.style.boxSizing = "border-box";
  }
  ...
  function onSpaNavigation() { clearTimeout(timer); window.requestAnimationFrame(patch); }
  function init() {
    patch();
