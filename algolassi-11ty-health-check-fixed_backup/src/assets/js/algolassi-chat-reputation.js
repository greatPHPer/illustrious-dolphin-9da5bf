...
(function () {
  ...
  async function decorate() {
    ...
    var allDecorated = nodes.every(function (node) { return !!node.querySelector(".algolassi-chat-reaction-row"); });
    ...
    try {
      ...
      nodes.forEach(function (node, index) {
        ...
        if (meta) {
          ...
          if (strong) {
            var line = document.createElement("span"); line.className = "algolassi-chat-author-line";
            var name = document.createElement("strong"); name.textContent = strong.textContent;
            strong.replaceWith(line); line.appendChild(name);
            var p = profileByName[row.username];
            var rep = document.createElement("span"); rep.className = "algolassi-chat-reputation";
            rep.textContent = "⭐ " + String(p && Number.isFinite(Number(p.reputation)) ? Number(p.reputation) : 0);
            line.appendChild(rep);
          }
        }
        var oldRow = node.querySelector(".algolassi-chat-reaction-row"); if (oldRow) oldRow.remove();
        var reactionRow = document.createElement("div"); reactionRow.className = "algolassi-chat-reaction-row";
        var button = document.createElement("button"); button.type = "button"; button.className = "algolassi-chat-like" + (mine[row.id] ? " is-reacted" : "");
        button.textContent = "👍"; button.setAttribute("aria-label", mine[row.id] ? "Remove positive reaction" : "Give positive reaction"); button.setAttribute("title", mine[row.id] ? "Remove positive reaction" : "Give positive reputation"); button.disabled = !myId;
        var count = document.createElement("span"); count.className = "algolassi-chat-reaction-count"; count.textContent = String(counts[row.id] || 0);
        button.addEventListener("click", async function () {
          button.disabled = true;
          var rpc = await client.rpc("toggle_chat_positive_reaction", { p_message_id: row.id });
          if (rpc.error) { console.error("Algolassi reputation reaction:", rpc.error); button.disabled = false; return; }
          var data = Array.isArray(rpc.data) ? rpc.data[0] : rpc.data;
          var reacted = !!(data && data.reacted); var newCount = data && data.reaction_count != null ? data.reaction_count : 0;
          button.textContent = "👍"; button.classList.toggle("is-reacted", reacted); button.setAttribute("aria-label", reacted ? "Remove positive reaction" : "Give positive reaction"); button.setAttribute("title", reacted ? "Remove positive reaction" : "Give positive reputation"); count.textContent = String(newCount); button.disabled = false; lastSignature = ""; decorate();
        });
        reactionRow.appendChild(button); reactionRow.appendChild(count); node.appendChild(reactionRow);
      });
      lastSignature = signature;
    } finally { decorating = false; }
  }

  function refreshAfterVisibilityChange() { lastSignature = ""; setTimeout(function () { decorate(); }, 50); }

  function start() {
    if (initialized) return; initialized = true; addStyles();
    var observer = new MutationObserver(function () { setTimeout(decorate, 30); }); observer.observe(document.body, { childList: true, subtree: true });
    ...
    var tries = 0; var timer = setInterval(function () { decorate(); if (++tries > 30) clearInterval(timer); }, 1000);
  }
