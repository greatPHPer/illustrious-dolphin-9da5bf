...
(function(){
  ...
  function cropLayer(){
    ...
    var overlay=q('image-crop-overlay'),useCropSelection=!!(overlay&&!overlay.classList.contains('image-hidden'));
    var xField=q(useCropSelection?'crop-x':'image-layer-crop-x'),yField=q(useCropSelection?'crop-y':'image-layer-crop-y'),wField=q(useCropSelection?'crop-width':'image-layer-crop-width'),hField=q(useCropSelection?'crop-height':'image-layer-crop-height');
    var x=Math.max(0,Math.min(l.canvas.width-1,parseInt(xField&&xField.value,10)||0)),y=Math.max(0,Math.min(l.canvas.height-1,parseInt(yField&&yField.value,10)||0)),w=Math.max(1,parseInt(wField&&wField.value,10)||1),h=Math.max(1,parseInt(hField&&hField.value,10)||1);
    w=Math.min(w,l.canvas.width-x);h=Math.min(h,l.canvas.height-y);
    var src=cloneCanvas(l.canvas),ctx=l.canvas.getContext('2d');ctx.clearRect(0,0,l.canvas.width,l.canvas.height);ctx.drawImage(src,x,y,w,h,x,y,w,h);S.active=false;setMagicColorUIVisible(false);clearSelection();render();draw();status('Cropped selected layer only.',true);
  }
