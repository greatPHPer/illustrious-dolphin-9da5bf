...
(function(){
  ...
  function open(){return ensureLayered().then(function(ok){
    ...
    showPanel();ensureMagicColorUI();setMagicColorUIVisible(false);draw();status('Layers ready. Select a layer and use its tools on the right.',true);
  });}
  ...
  function render(){
    ...
    for(var i=S.layers.length-1;i>=0;i--){(function(index){
      ...
      row.addEventListener('click',function(){setMagicColorUIVisible(false);S.active=false;S.current=index;clearSelection();syncLayerControls();syncLayerTransformFields();render();draw();});list.appendChild(row);
  ...
  function moveLayer(i,delta){var ni=i+delta;if(ni<0||ni>=S.layers.length)return;var t=S.layers[i];S.layers[i]=S.layers[ni];S.layers[ni]=t;S.current=ni;S.active=false;setMagicColorUIVisible(false);render();draw();}
  function addLayer(){if(!S.width)return;S.active=false;setMagicColorUIVisible(false);var c=blankCanvas();S.layers.push({name:'Layer '+(S.layers.length+1),visible:true,opacity:1,canvas:c});S.current=S.layers.length-1;clearSelection();render();draw();}
  function duplicateLayer(){var l=selected();if(!l)return status('Select a layer first.',false);S.active=false;setMagicColorUIVisible(false);S.layers.splice(S.current+1,0,{name:l.name+' copy',visible:l.visible,opacity:l.opacity,canvas:cloneCanvas(l.canvas)});S.current++;clearSelection();render();draw();status('Layer duplicated.',true);}
  function deleteLayer(){if(S.layers.length<=1)return status('Keep at least one layer.',false);S.active=false;setMagicColorUIVisible(false);S.layers.splice(S.current,1);S.current=Math.max(0,Math.min(S.current,S.layers.length-1));clearSelection();render();draw();}
  ...
  function cropLayer(){var l=selected();if(!l)return status('Select a layer first.',false);var x=Math.max(0,Math.min(l.canvas.width-1,parseInt(q('image-layer-crop-x').value,10)||0)),y=Math.max(0,Math.min(l.canvas.height-1,parseInt(q('image-layer-crop-y').value,10)||0)),w=Math.max(1,parseInt(q('image-layer-crop-width').value,10)||1),h=Math.max(1,parseInt(q('image-layer-crop-height').value,10)||1);w=Math.min(w,l.canvas.width-x);h=Math.min(h,l.canvas.height-y);var src=cloneCanvas(l.canvas),ctx=l.canvas.getContext('2d');ctx.clearRect(0,0,l.canvas.width,l.canvas.height);ctx.drawImage(src,x,y,w,h,x,y,w,h);S.active=false;setMagicColorUIVisible(false);clearSelection();render();draw();status('Cropped selected layer only.',true);}
  function scaleLayer(){var l=selected();if(!l)return status('Select a layer first.',false);var nw=Math.max(1,parseInt(q('image-layer-scale-width').value,10)||l.canvas.width),nh=Math.max(1,parseInt(q('image-layer-scale-height').value,10)||l.canvas.height),src=cloneCanvas(l.canvas),out=newCanvas(S.width,S.height),ctx=out.getContext('2d');ctx.drawImage(src,(S.width-nw)/2,(S.height-nh)/2,nw,nh);l.canvas=out;S.active=false;setMagicColorUIVisible(false);clearSelection();render();draw();syncLayerTransformFields();status('Scaled selected layer only.',true);}
  function rotateLayer(deg){var l=selected();if(!l)return status('Select a layer first.',false);var src=cloneCanvas(l.canvas),oldW=src.width,oldH=src.height,newW=oldH,newH=oldW,out=newCanvas(newW,newH),ctx=out.getContext('2d');ctx.translate(newW/2,newH/2);ctx.rotate(deg*Math.PI/180);ctx.drawImage(src,-oldW/2,-oldH/2);l.canvas=out;var targetW=Math.max(S.width,newW),targetH=Math.max(S.height,newH);if(targetW>S.width||targetH>S.height){var oldDocW=S.width,oldDocH=S.height,dx=Math.round((targetW-oldDocW)/2),dy=Math.round((targetH-oldDocH)/2);S.layers.forEach(function(layer){if(layer===l){var nc=newCanvas(targetW,targetH),nctx=nc.getContext('2d');nctx.drawImage(layer.canvas,Math.round((targetW-newW)/2),Math.round((targetH-newH)/2));layer.canvas=nc;}else{resizeLayerCanvas(layer,targetW,targetH,dx,dy);}});S.width=targetW;S.height=targetH;}else if(l.canvas.width!==S.width||l.canvas.height!==S.height){var centered=newCanvas(S.width,S.height);centered.getContext('2d').drawImage(l.canvas,(S.width-newW)/2,(S.height-newH)/2);l.canvas=centered;}S.active=false;setMagicColorUIVisible(false);clearSelection();render();draw();syncLayerTransformFields();status('Rotated layer “'+l.name+'” only ('+deg+'°).',true);}
  ...
  function resetLayerState(){setMagicColorUIVisible(false);S.layers=[];S.current=-1;S.width=0;S.height=0;S.selection=null;S.selectionCanvas=null;S.active=false;var stage=q('image-preview-stage');if(stage)stage.classList.remove('image-layers-active');var c=q('image-layers-canvas'),sel=q('image-layers-selection');if(c)c.classList.add('image-hidden');if(sel)sel.classList.add('image-hidden');}
