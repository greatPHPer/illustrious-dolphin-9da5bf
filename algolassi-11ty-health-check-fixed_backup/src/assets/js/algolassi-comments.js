(function () {
  ...
  function getSession() {
    ...
    return window.AlgolassiSupabase.auth.getSession().then(function (r) {
      return r && r.data && r.data.session ? r.data.session : null;
    }).catch(function (e) {
      console.warn("Algolassi comment auth lookup:", e);
      return null;
    });
  }
  ...
  function getUsername(user) {
    ...
    return window.AlgolassiSupabase.from("profiles").select("username").eq("user_id", user.id).maybeSingle().then(function (r) {
      return !r.error && r.data ? (r.data.username || "") : "";
    }).catch(function (e) {
      console.warn("Algolassi comment profile lookup:", e);
      return "";
    });
  }
  ...
  function initComments() {
    ...
    function loadComments() {
      ...
      return fetchWithTimeout(url, { headers: headers }).then(function (response) {
    ...
    form.addEventListener("submit", function (event) {
      ...
      if (!name || !comment) {
        if (status) status.textContent = "Please enter your name and comment.";
        return;
      }
      ...
      getSession().then(function (session) {
        ...
        if (session && session.user && window.AlgolassiSupabase) {
          return window.AlgolassiSupabase.from("comments").insert(payload).then(function (result) {
            ...
            if (status) status.textContent = "Thanks! Your comment is now published.";
          });
        }
        return fetchWithTimeout(ENDPOINT, { method: "POST", headers: Object.assign({}, headers, { Prefer: "return=minimal" }), body: JSON.stringify(Object.assign({}, payload, { approved: false })) }).then(function (response) {
          if (response.ok) return;
          return response.text().then(function (text) {
            throw new Error("Comments POST failed: HTTP " + response.status + (text ? " " + text : ""));
          });
        }).then(function () {
          form.reset();
          return updateIdentity();
        }).then(function () {
          if (status) status.textContent = "Thanks! Your comment was submitted and will appear after approval.";
        });
