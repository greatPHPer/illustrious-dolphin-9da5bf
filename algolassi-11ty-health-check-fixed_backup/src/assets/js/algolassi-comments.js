(function () {
  ...
  function getUsername(user) {
    ...
    return window.AlgolassiSupabase.from("profiles")
      .select("username")
      .eq("user_id", user.id)
      .maybeSingle()
        ...
        .then(function (items) {
          ...
          list.innerHTML = items.map(function (item) {
            return '<article class="comment-item">' +
              ...
              '<span class="comment-date">' + escapeHtml(new Date(item.created_at).toLocaleString()) + '</span></div>' +
    ...
    form.addEventListener("submit", function (event) {
      ...
      return getSession().then(function (session) {
        ...
        return fetchWithTimeout(ENDPOINT, {
          ...
          body: JSON.stringify(Object.assign({}, payload, { approved: false }))
        }).then(function (response) {
          if (!response.ok) throw new Error(awaitResponseText(response));
          form.reset();
          return updateIdentity();
        }).then(function () {
  ...
  function awaitResponseText(response) {
    return response.text().then(function (text) {
      return "Comments POST failed: HTTP " + response.status + (text ? " " + text : "");
    });
  }
