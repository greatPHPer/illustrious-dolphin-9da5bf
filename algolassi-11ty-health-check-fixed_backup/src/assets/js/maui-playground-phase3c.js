...
(function () {
  ...
  /* Compatibility renderer for the Razor demos that regressed after the playground renderer was expanded. */
  function startRazorCompatibility() {
    var affected = {
    ...
    var timer = setInterval(function () {
      ...
      if (!affected[demo] || !window.MauiPlayground) {
      ...
      function renderCompat() {
        ...
        var codeMatch = /@code\s*\{([\s\S]*)\}\s*$/i.exec(source);
        var code = codeMatch ? codeMatch[1] : "";
        var state = window.MauiPlayground.stateFor(code);
        var markup = codeMatch ? source.slice(0, codeMatch.index) : source;
        markup = markup.replace(/@page\s+["'][^"']+["']/gi, "");
        markup = markup.replace(/@using\s+[^\r\n]+/gi, "");

        markup = markup.replace(/<(RadzenButton|MudButton)\b([^>]*)\/>/gi, function (_, tag, raw) {
          var a = parseAttrs(raw);
          var click = a.OnClick || a["@onclick"] || a.onclick || "";
          var text = a.Text || "Click me";
          return '<button type="button" class="playground-' + tag.toLowerCase() + '" data-playground-click="' + esc(click.replace(/^@/, "")) + '">' + esc(text) + '</button>';
        });
        markup = markup.replace(/<(MudTextField)\b([^>]*)\/>/gi, function (_, tag, raw) {
          var a = parseAttrs(raw);
          return '<input type="text" class="playground-mud-input" placeholder="' + esc(a.Label || a.Placeholder || "") + '" value="' + esc(a.Value || "") + '">';
        });
        markup = markup.replace(/@onclick\s*=\s*["']([^"']+)["']/gi, 'data-playground-click="$1"');
        markup = markup.replace(/@([A-Za-z_]\w*)/g, function (all, name) {
          return Object.prototype.hasOwnProperty.call(state, name) ? esc(state[name]) : all;
        });
        ...
        content.querySelectorAll("[data-playground-click]").forEach(function (button) {
          button.addEventListener("click", function (event) {
            ...
            var name = button.getAttribute("data-playground-click").replace(/^@/, "").trim();
            try {
              window.MauiPlayground.executeMethod(name, code, state);
              renderCompat();
            } catch (error) {
              var output = document.getElementById("maui-console-output");
      ...
      function parseAttrs(raw) {
        ...
        var re = /([:@\w.-]+)\s*=\s*["']([^"']*)["']/g;
        var m;
        while ((m = re.exec(raw || ""))) result[m[1]] = m[2];
        return result;
      }

      window.MauiPlayground.render = renderCompat;
      renderCompat();
      clearInterval(timer);
    }, 100);
  }

  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", startRazorCompatibility);
  else startRazorCompatibility();
  window.addEventListener("algolassi:spa-navigation", startRazorCompatibility);
})();
