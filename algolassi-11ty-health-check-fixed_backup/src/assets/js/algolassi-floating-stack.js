...
(function () {
  ...
  function newsToast(){var h=assistantHost();return h?visibleRect(h.querySelector(".algolassi-assistant-news")):null;}
  ...
  function ensureNewsButton(toast){if(!toast||applyingNews)return;var head=toast.querySelector(".algolassi-assistant-head");if(!head||head.querySelector(".algolassi-news-hide"))return;applyingNews=true;try{var b=document.createElement("button");b.type="button";b.className="algolassi-assistant-action algolassi-news-hide";b.setAttribute("aria-label","Hide news");b.title="Hide news";b.textContent="✕ Hide";b.style.cssText="margin-left:auto;border:0;border-radius:7px;background:transparent;color:inherit;font-size:12px;line-height:1;cursor:pointer;padding:5px 8px";b.addEventListener("click",function(e){e.preventDefault();e.stopPropagation();hideNewsToast(toast);},false);head.appendChild(b);}finally{applyingNews=false;}}
  ...
  function updateAll(){
    syncNewsLauncher();
    var news=newsToast(), nmini=newsLauncher(), rbEl=document.getElementById("algolassi-radio-reopen"), rb=radioLauncher(), rc=radioCard(), cbEl=document.getElementById("algolassi-chat-reopen-button"), cb=chatLauncher();
    /* Build one deterministic stack from the bottom upward. */
    ...
    var cc=chatCard(), ch=document.getElementById("algolassi-chat-presence-host");
    if(cc&&ch)setBottom(ch,cursor);
  }
  function settle(){updateAll();[40,100,180,300,500].forEach(function(d){setTimeout(updateAll,d);});}
  function start(){settle();window.addEventListener("resize",settle,{passive:true});window.addEventListener("scroll",updateAll,{passive:true});window.addEventListener("algolassi:radio-layout-change",settle);window.addEventListener("algolassi:spa-navigation",settle);window.addEventListener("algolassi:news-layout-change",settle);window.addEventListener("algolassi:news-reopen",settle);
